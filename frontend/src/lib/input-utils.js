...
export function getFriendlyApiError(error, fallbackMessage = 'Upload failed — please check your connection') {
  ...
  if (responseMessage) {
    if (/groq_api_key|api key/i.test(responseMessage)) {
      return 'Server configuration error. Contact admin to set GROQ_API_KEY in Vercel.';
    }

    if (/quota|billing|rate limit|resource exhausted|credit/i.test(responseMessage)) {
      return 'Groq request limit reached. Try again later or check the API project billing settings.';
    }
