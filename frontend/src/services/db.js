...
export async function saveAnalysis(userId, resumeData, rawText) {
...
export async function saveJobMatch(analysisId, userId, jobTitle, companyName, jobDescription, matchResult) {
  const supabase = getSupabaseClient();
  const jobMatch = {
    id: createId('match'),
...
export async function deleteJobMatchById(id, userId) {
  const supabase = getSupabaseClient();
...
export async function seedDemoDataIfEmpty(userId = 'demo-user') {
  const supabase = getSupabaseClient();
...
export async function getUserStats(userId) {
  ...
  const analysisMappedError = mapSupabaseError(analysisError, 'Failed to fetch analysis stats.');
  if (analysisMappedError) {
    throw analysisMappedError;
  }
  ...
  const matchMappedError = mapSupabaseError(matchError, 'Failed to fetch match stats.');
  if (matchMappedError) {
    throw matchMappedError;
  }
